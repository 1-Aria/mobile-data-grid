...
// --- HELPER FUNCTIONS & MEMOIZED COMPONENTS (HOISTED FOR PERFORMANCE) ---

/**
 * HOISTED 1/4: copyToClipboard helper is moved outside IncidentRow.
 * This prevents the function from being redefined on every row render.
 */
...
/**
 * HOISTED 2/4: DetailField component is moved outside IncidentRow.
 * This prevents the component definition from being recreated on every row render.
 * Note: Styling reflects the final approved version (blue, bold, text-sm).
 */
const DetailField = ({ label, value, colorClass = 'text-gray-700' }: { label: string, value: string | React.ReactNode, colorClass?: string }) => (
    <div className="flex flex-col mb-3">
        {/* Updated: text-sm, font-bold, text-indigo-700 for theme/emphasis */}
        <span className="text-sm font-bold uppercase text-indigo-700">{label}</span>
        
        {/* Value remains text-sm, using mt-0.5 for small vertical separation */}
...
/**
 * HOISTED 3/4: IncidentRow is wrapped in React.memo.
 * This prevents unnecessary re-renders of all rows when only the expandedId state changes.
 */
...
      {/* EXPANDED VIEW (Full Details) - Uses the external DetailField component */}
      {isExpanded && (
...
  // Toggle function passed down to children (stable reference)
...
  
  // HOISTED 4/4: Counts are memoized with useMemo.
  // This ensures they are only recalculated when allIncidents actually changes, 
  // not on every state change (like searchTerm or statusFilter).
...
                
                {/* Status Counts (Now use Memoized values) */}
